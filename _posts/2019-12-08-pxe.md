...
```
# if you use virt-install for install vm, dhcp is not required.
...
```
# set kernel image, initrd, kickstart file in default
kernel vmlinuz
...
```
# I have no metalware, so i use virt-install to install vm
...
with virt-install, you even don't need a tftp server, just use a http server, and put all relevant cfg into http server.
