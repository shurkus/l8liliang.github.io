...
/plan:
    discover:
        ...
        #path: /networking/switchdev-selftest
        #url: https://gitlab.com/liali666/tmt-test
        # 这个url和path用来寻找测试(plan,test etc...)，但是测试执行的时候，还是在当前repo(本文件所在repo)下执行。
...
testing-farm request --compose RHEL-9.2.0-Nightly --git-url https://gitlab.cee.redhat.com/liali/kernel.git  --plan /networking/switchdev-sleftest/plan --hardware hostname=mlxsw-sn2100-01.mgmt.lab.eng.pek2.redhat.com
```
