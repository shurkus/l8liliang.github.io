...
## 概览
scrapy是一个python爬虫框架，可以从网站上爬取数据。  
...
把上面的代码保存的一个文件中，比如quotes_spider.py，然后使用runspider命令运行这个爬虫。
```
...
### 发生了什么
当你运行scrapy runspider quotes_spider.py时，scrapy找到该文件中定义的爬虫（QuotesSpider），然后通过crawler引擎运行这个爬虫。  
crawl首先向start_urls指定的地址发送请求（request），然后使用默认的回调方法parse（response）处理响应。  
在parse方法中，我们可以使用css或者xpath从response中提取我们感兴趣的数据，保存起来（也可以直接打印出来）。    
然后寻找下一个连接，针对该连接重复上面的过程。  
