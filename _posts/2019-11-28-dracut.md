...
udev(userspace device)是一个用户态的工具，可以监听硬件设备的状态，然后根据你设置的rules去执行一些相应的动作。
...
-a 选项，      添加你想要的dracut modules到initramfs中，多个module已空格分隔
-m 选项，      只安装这些dracut modules到initramfs中，多个module已空格分隔
--drivers，    只安装这些driver（driver指的是内核模块，和dracut module不同，后者是dracut自己的东西）
--add-drivers，除了安装默认driver，还安装这些drivers

# 通过--include选项注入自己的脚本
# dracut --include cmdline-preset /etc/cmdline.d/mycmdline.conf initramfs-cmdline-pre.img #把cmdline-present拷贝到initramfs的/etc/cmdline.d/mycmdline.conf
